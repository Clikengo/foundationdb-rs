...
impl Single for f32 {
    fn encode<W: Write>(&self, mut w: W) -> std::io::Result<()> {
        w.write_all(&[0x020])?;
        let buf: [u8; 4] = unsafe { std::mem::transmute(*self) };
        w.write_all(&buf)
    }
    ...
    fn decode(buf: &[u8]) -> Result<(Self, usize)> {
        ...
        let val: f32 = unsafe { std::mem::transmute(data) };
        Ok((val, 5))
    }
}
...
impl Single for f64 {
    fn encode<W: Write>(&self, mut w: W) -> std::io::Result<()> {
        w.write_all(&[0x021])?;
        let buf: [u8; 8] = unsafe { std::mem::transmute(*self) };
        w.write_all(&buf)
    }
    ...
    fn decode(buf: &[u8]) -> Result<(Self, usize)> {
        ...
        let val: f64 = unsafe { std::mem::transmute(data) };
        Ok((val, 9))
    }
}
